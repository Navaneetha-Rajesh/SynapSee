...
import { triggerN8NWebhook } from './src/services/n8n';
...
app.post('/api/medications/compliance', async (req, res) => {
  ...
  // Trigger n8n check
  try {
    await triggerN8NWebhook('medication-check', {
      medId,
...
app.post('/api/voice/process', upload.single('file'), async (req, res) => {
  ...
  try {
    n8nResult = await triggerN8NWebhook<any>('stt-analyze', {
      audio: base64Audio,
...
app.get('/api/games/data/:gameKey', async (req, res) => {
  ...
  try {
    n8nResult = await triggerN8NWebhook<any>('generate-game', { gameKey });
  } catch (err) {
...
app.post('/api/games/log', async (req, res) => {
  ...
  if (isSupabaseActive()) {
    try {
      await supabase!.from('game_logs').insert(gameLog);
    } catch (e) {
  ...
  try {
    const n8nResult = await triggerN8NWebhook<any>('game-analytics', {
      patientId: patientId || 'user-eleanor',
      gameKey,
      durationSeconds,
      correctCount,
...
app.get('/api/v1/game-logs', async (req, res) => {
  if (isSupabaseActive()) {
    try {
      const { data, error } = await supabase!.from('game_logs').select('*');
      if (!error && data) {
        return res.json(data);
      }
    } catch (e) {
...
app.get('/api/v1/patient/analytics', async (req, res) => {
  if (isSupabaseActive()) {
    try {
      const { data, error } = await supabase!.from('patient_analytics').select('*');
      if (!error && data) {
        return res.json(data);
      }
    } catch (e) {
